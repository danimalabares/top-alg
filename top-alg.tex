...
\section{Simplejos}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{Homología/H2.1.1}
	\end{figure}
...
Una cara de $[v_0,...,v_n]$ es el subsimplejo de generado por cualquier subconjunto no vacío de ${v_0,...,v_n}$. Cualquier cara de dimensión 1 $[v_i,v_j]$ vamos a considerarla orientada así
\begin{tikzpicture}
	\node (vi) at (0,0) {$v_i$};
	\node (vj) at (2,0) {$v_j$};
	\draw [->] (vi) -- (vj);
