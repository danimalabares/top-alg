...
\end{defn}
\begin{prop}
	Sean $p:(\tilde{X},\tilde{x}_0)\to (X,x_0)$ un cubriente y una función $f:(Y,y_0)\to (X,x_0)$ con $Y$ espacio arco-conexo y localmente arco-conexo. Entonces existe un levantamiento $\tilde{f}:(Y,y_0)\to(\tilde{X},x_0)$ si y sólo si $f_*\left(\pi_1(Y,y_0)\right)\subseteq p_*\left(\pi_1(\tilde{X},x_0)\right)$.
\end{prop}
...
\section{Transfromaciones de cubierta}
	...
	\end{prop}
\section{Acciones de grupos y cubrientes}
\begin{defn}
...
\end{proof}
\begin{ejer}
	Para el sábado: hacer de homología y quizás uno de grupo fundamental.
\end{ejer}
\part{Homología}
